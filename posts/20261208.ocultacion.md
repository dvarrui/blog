...
Su objetivo no es el "secreto" por seguridad, sino la abstracción: que el resto del programa no dependa de cómo guardas los datos, sino de qué puede hacer el objeto por ti.
...
Podríamos decir que tenemos los siguientes tipos de ocultación:
...
Tenemos tanto los objetos como las clases se basan en diccionarios:
...
Persona.saludar(usuario)    #=> Se resuelve el azúcar sintáctico como uma llamada de funciones
...
Es necesario que no exista ocultación en el lenguaje para que funcione la creación de atributos específicos del objeto en ejecución. Ejemplo
```python
class Persona:
